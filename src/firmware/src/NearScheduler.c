...
void nearSchedulerAdd(const struct NearSchedule *const schedule)
{
	...
	schedules[0].eventType = schedule->eventType;
	schedules[0].eventArgs = schedule->eventArgs;
	if (!NCO1CONbits.N1EN)
	{
		NCO1ACCU = 0;
		NCO1ACCH = 0;
...
static void onWokenFromSleep(const struct Event *const event)
{
	...
	PIR7bits.NCO1IF = 0;
	if (++ticks == schedules[0].ticks)
		eventPublish(schedules[0].eventType, schedules[0].eventArgs);
}
